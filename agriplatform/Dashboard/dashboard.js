...
function renderCrops(){
  ...
  items.forEach((c,i)=>{
    ...
    <div class="crop-actions">
      <button class="btn small" data-i="${i}" data-act="view">View</button>
      <button class="btn small" data-i="${i}" data-act="up">+5%</button>
      <button class="btn small" data-i="${i}" data-act="down">-5%</button>
      <button class="btn small" data-i="${i}" data-act="del">Remove</button>
    </div>`;
    list.appendChild(el);
  });
  list.querySelectorAll('button').forEach(btn=>{
    btn.onclick=()=>{
      ...
      if(act==='view'){ openCropModal(i); return; }
...
let LAST_WEATHER=null, LAST_RAIN=null;

function computeGrowth(crop){
  // Days since planted
  let days=30;
  if(crop.planted){
    const d=new Date(crop.planted); if(!isNaN(d)){
      days=Math.max(0, Math.floor((Date.now()-d.getTime())/86400000));
    }
  }
  // Temperature driver using current temp if available
  const t = LAST_WEATHER?.current?.temp ?? LAST_WEATHER?.daily?.[0]?.temp?.day ?? 25;
  // Growing degree day approx with base 10C and cap 30C
  const gdd = Math.max(0, Math.min(30, t) - 10);
  // Moisture factor from last month rainfall
  const lastRain = Array.isArray(LAST_RAIN?.values) ? LAST_RAIN.values[LAST_RAIN.values.length-1] : 50;
  // Assume ideal monthly rainfall ~120mm, clamp 0..1
  const moisture = Math.max(0, Math.min(1, lastRain/120));
  // Simple growth model: progress over 120 days with climate multipliers
  const climateBoost = (0.6 + (gdd/20)*0.3) * (0.7 + moisture*0.3);
  let progress = Math.min(100, Math.round((days/120)*100*climateBoost));
  const stage = progress<25? 'Germination' : progress<60? 'Vegetative' : progress<85? 'Flowering' : 'Maturity';
  return {progress, stage, days, temp: t, lastRain};
}

let modalState={index:null, computed:null};
function openCropModal(i){
  const items=readCrops(); const c=items[i]; if(!c) return;
  const r=computeGrowth(c); modalState={index:i, computed:r};
  const body=document.getElementById('cropModalBody');
  document.getElementById('cropModalTitle').textContent = `${c.name} • Recommendations`;
  body.innerHTML = `
    <div class="muted">Planted: ${c.planted||'-'} · Area: ${c.area||'-'}</div>
    <div style="margin-top:8px">Estimated Growth: <strong>${r.progress}%</strong> (${r.stage})</div>
    <div class="muted" style="margin-top:6px">Drivers: Temp ${r.temp.toFixed(1)}°C, Last month rain ${Math.round(r.lastRain||0)}mm, Days since planted ${r.days}</div>
    <div style="margin-top:10px">Advice:
      <ul>
        <li>Ensure irrigation if rainfall < 90mm last month.</li>
        <li>Apply balanced NPK guided by soil NPK bars above.</li>
        <li>Monitor pests during ${r.stage.toLowerCase()} stage.</li>
      </ul>
    </div>`;
  document.getElementById('cropModal').hidden=false;
}

function closeCropModal(){ document.getElementById('cropModal').hidden=true; modalState={index:null,computed:null}; }
...
// Modal wiring
document.getElementById('closeModal')?.addEventListener('click', closeCropModal);
document.getElementById('cropModal')?.addEventListener('click', (e)=>{ if(e.target.id==='cropModal') closeCropModal(); });
document.getElementById('syncProgress')?.addEventListener('click', ()=>{
  if(modalState.index==null || !modalState.computed) return;
  const items=readCrops();
  items[modalState.index].progress = modalState.computed.progress;
  writeCrops(items); renderCrops(); closeCropModal();
});
